...
VERSION=$(cat version)
IMAGE_NAME="abhijitwakchaure/example-db"
...
echo -e "\nBuilding go code..."
CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o db-server .
...
docker run --rm -it -p 8081:8081 ${IMAGE_NAME}:${VERSION}
